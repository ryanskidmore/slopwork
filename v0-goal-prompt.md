...
**Dogfood flip (M2)**: the moment B1–B4 + C1–C3 + D1 have landed, run `slop init` here and file every remaining work item as a real Slopwork ticket with proper edges (E3, pulled early). From then on, `slop ready` picks the next dispatch; `start`/`done` tickets as subagents complete them; file discoveries with `--discovered-from`. Before the flip, use your todo list.
